...
export default function Dashboard() {
  ...
  const pressSimpan = async () => {
    ...
    try {
      let response = await laboranService({ web3, contract }).tambah(formData);

      alert(response);
      router.push(`/admin/riwayat-medis/laboran/list/${paramCheckup}`);
    } catch (error) {
  ...
  const getUser = async () => {
    try {
      ...
      if (!response.roles.includes("laboran")) {
        alert("Anda bukan Laboran");
